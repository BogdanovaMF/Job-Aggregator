...
if __name__ == '__main__':
    ...
    job_hh = requests.get(url.format(specialization=args["specialization"], num_page='page=0'), headers=headers)
    ...
    for i in range(total_pages):
        links += get_links(url.format(specialization=args["specialization"], num_page=f'page={i}'))
        logger.info(f'Page №{i + 1}/{total_pages}: {args["specialization"].replace("-", " ")}')
        time.sleep(0.7)
    ...
    save_data(
        ...
        output_filepath=OUTPUT_FILEPATH_TEMPLATE.format(source_type='hh',
                                                        specialization=args["specialization"].replace('-', '_'),
